...
const int rampStep   = 2;     // PWM increment per loop
...
float Kp = 70.0;   // start values – you will tune these
float Ki = 0.0;
float Kd = 12.0;
...
// Target base speed (what you want during line following)
...
float getLineError() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    value = 4095 - value;    // ESP32 12-bit ADC (0–4095) TAKE THIS OUT ? 
  ...
  if (denominator == 0) {
    // No line detected – keep previous error
    return previousError;
  }

  return (float)numerator / (float)denominator;  // range approx -2 … +2
}
