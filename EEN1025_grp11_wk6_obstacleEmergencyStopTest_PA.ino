...
bool parking = false;
bool obstacleDetected = false;  // obstacle-triggered early finish
unsigned long ultrasonicIgnoreUntilMs = 0; // warm-up window after boot/route fetch
unsigned long lastDistancePrintMs = 0;       // periodic distance logging
const unsigned long DIST_PRINT_INTERVAL_MS = 25UL; // 0.5s sampling/printing
const int DIST_MEDIAN_WINDOW = 3;                   // last 3 readings
const float EMERGENCY_MEDIAN_CM = 17.0;              // trigger if median <= this (non-parking)
float lastDistances[DIST_MEDIAN_WINDOW] = {0};
int distIdx = 0;
int distCount = 0;
int finalPos;
//===========================================================
...
void emergencyFinishFromObstacle(float distanceCm);
...
CaseState intToCaseState(int v) {
  ...
  return CASE_0; // default fallback for unexpected values
}
...
void emergencyFinishFromObstacle(float distanceCm) {
  ...
  Serial.println(" cm. Emergency finishing route.");

  // Prevent any further case/node logic from running
  routeIndex = routeLength;
  actionInProgress = true;

  // Stop immediately, turn around, then stop again
  Brake(0);
  flip180();
  Brake(0);
...
void setup() {
  ...
  // Ignore ultrasonic for a short warm-up period to avoid boot-time false triggers
  ultrasonicIgnoreUntilMs = millis() + 1200;
  lastDistancePrintMs = millis();
  distIdx = 0;
  distCount = 0;
...
void loop() {
  ...
  // Continuous obstacle monitoring (except during parking routine)
  // Solution B: warm-up ignore window to avoid boot-time false triggers
  bool ultrasonicReady = (millis() >= ultrasonicIgnoreUntilMs);

  // "Parking phase" is when the route intends to park (CASE_1 with nextCase == CASE_1 and parking flag set)
  bool parkingPhase = (parking && routeIndex < routeLength && currentCase == CASE_1 && route[routeIndex] == CASE_1);

  if (ultrasonicReady && !parkingPhase) {
    // Sample on the same cadence as the periodic print (0.5s)
    if (millis() - lastDistancePrintMs >= DIST_PRINT_INTERVAL_MS) {
      lastDistancePrintMs = millis();

      float distanceCm = readUltrasoundCm();

      // Store into circular buffer
      lastDistances[distIdx] = distanceCm;
      distIdx = (distIdx + 1) % DIST_MEDIAN_WINDOW;
      if (distCount < DIST_MEDIAN_WINDOW) distCount++;

      Serial.print("Ultrasonic distance (cm): ");
      Serial.println(distanceCm);

      // Only evaluate emergency stop once we have 3 readings
      if (distCount >= DIST_MEDIAN_WINDOW) {
        float a = lastDistances[0];
        float b = lastDistances[1];
        float c = lastDistances[2];
        ...
        Serial.print("Ultrasonic median last 3 (cm): ");
        Serial.println(median);

        if (median <= EMERGENCY_MEDIAN_CM) {
          emergencyFinishFromObstacle(median);
          return;
        }
      }
    }
  } else {
    // During warm-up or parking phase, do not accumulate readings for emergency-stop mean
    distIdx = 0;
    distCount = 0;

    // Still print periodically
    if (millis() - lastDistancePrintMs >= DIST_PRINT_INTERVAL_MS) {
      lastDistancePrintMs = millis();
      float distanceCm = readUltrasoundCm();
      Serial.print("Ultrasonic distance (cm): ");
