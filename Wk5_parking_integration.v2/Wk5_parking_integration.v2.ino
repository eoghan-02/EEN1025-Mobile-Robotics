...
// ======================= WIFI + SERVER CONFIG =======================
static char ssid[]     = "Eoghans iPhone";
...
void driveStraightUntilObstacle() {
    int driveSpeed = baseSpeedTarget; // half normal speed
    ...
    while (true) {
        ...
        analogWrite(motor1PWM, driveSpeed);
        analogWrite(motor2PWM, driveSpeed);
...
void advanceCase(CaseState nextCase, bool enteringFromStart) {
  ...
  if (currentCase == CASE_5) {
    sendFinishedToServer((int)currentCase);  
    stopAndHalt();
  }
}
